...
ELSE
   ...
   THEN
      ...
      /*This check for PHONE or EMAIL only updates*/
      /*
      IF EXISTS (SELECT ssn, phone, email FROM transducer._SPERSON WHERE ssn = NEW.ssn
                  EXCEPT (SELECT NEW.ssn, NEW.phone, NEW.email))
      THEN
